...
densDat <- data.frame(Beta = c(rowMeans(ALL.dat, na.rm=TRUE), rowMeans(CTRL.dat, na.rm=TRUE)), type = rep(c("ALL", "CTRL"), each = nrow(CTRL.dat)))

densityplot(~Beta, densDat, groups=type, auto.key=TRUE, type=c("l", "g"))

# Using base graphics 

# plot(density(rowMeans(ALL.dat, na.rm=TRUE)))

# lines(density(rowMeans(CTRL.dat, na.rm=TRUE)))
...
(M.boxplot <- ggplot(data = M.CGI.tall, aes(Sample, M, fill = factor(Group))) + geom_boxplot() + ggtitle("Distribution of CGI M values") + xlab("Samples") + ylab("M values") + theme_bw() + scale_x_discrete(labels = NULL))
...
heatmap.2(DMR.CGI, col = redblue(256), RowSideColors = col, density.info = "none", trace = "none", Rowv = TRUE, Colv = TRUE, labCol = FALSE, labRow = FALSE, dendrogram = "row", keysize = 1)
...
# plot location of differential methylated probes along each chromosome get
# the length of chromosome 1-22 and X
...
(coord.plot <- ggplot(data = coordDMRprobe) + geom_linerange(aes(factor(chr, levels = c("X", as.character(22:1))), ymin = 0, ymax = length), data = chrlen, alpha = 0.5) + geom_point(aes(x = factor(chr, levels = c("X", as.character(22:1))), y = coord), position = position_jitter(width = 0.03), na.rm = T) + ggtitle("DMR positions on chromosomes") + ylab("Position of DMRs") + xlab("chr") + coord_flip() + theme_bw())
