...
for (i in 1:nfold) {
  ...
  for (kk in c(1, 5, 10, 15)) {
    ...
    yhat.knn <- knn(train = t(traindat.fold), test = t(testdat.fold), cl = trainclass.fold, k = kk)
...
####################### Exercise 1
# we can add for(i in 1:100) to splits <- GenerateLearningsets(y = train.es$LnStatus, method = "CV", fold = 6, strat = TRUE)
# we can add p.value = threshold in topTable
